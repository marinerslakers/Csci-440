...
--select * from Highschooler A where
--	A.ID in (select * from (select B.ID from Highschooler B, Friend C where
--			(B.ID = C.ID1 and 1709 = C.ID2)
--			or (B.ID = C.ID2 and 1709 = C.ID1)) D where
--		D.ID in (select E.ID from Highschooler F, Friend G where
--	and A.ID != 1709;
